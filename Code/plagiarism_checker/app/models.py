...
class SourceDocument(db.Model):
    ...
    format = db.Column(db.String(10), nullable=False)  # pdf, docx, txt
    size = db.Column(db.Integer, nullable=False)
    upload_date = db.Column(db.DateTime, default=db.func.current_timestamp())
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)

    user = db.relationship('User', back_populates='documents')
    processed_text = db.relationship('ProcessedText', back_populates='document', uselist=False)

class ProcessedText(db.Model):
    __tablename__ = 'processed_texts'
    id = db.Column(db.Integer, primary_key=True)
    doc_id = db.Column(db.Integer, db.ForeignKey('source_documents.id'), nullable=False)
    extracted_text = db.Column(db.Text)
    status = db.Column(db.String(20), default='pending')  # pending, completed, error

    document = db.relationship('SourceDocument', back_populates='processed_text')
    checks = db.relationship('PlagiarismCheck', back_populates='processed_text')

class PlagiarismCheck(db.Model):
    __tablename__ = 'plagiarism_checks'
    id = db.Column(db.Integer, primary_key=True)
    doc_id = db.Column(db.Integer, db.ForeignKey('processed_texts.id'), nullable=False)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
...
class Report(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    check_id = db.Column(db.Integer, db.ForeignKey('plagiarism_checks.id'), nullable=False)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
