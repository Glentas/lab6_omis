...
from app import db
from app.models import SourceDocument, Report
...
# Импортируем модели
from app import db
from app.models import SourceDocument, ProcessedText, PlagiarismCheck, Report  # <-- добавлен ProcessedText и другие
...
def upload():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            # Запуск предобработки в фоне (для MVP — можно и синхронно, но имитируем асинхронность)
            # Синхронная обработка (MVP)
...
def report_ready(doc_id):
    ...
    if doc.user_id != current_user.id:
        flash('Нет доступа')
        return redirect(url_for('student.dashboard'))
    # Находим последний отчёт по этому документу
    ...
    check = PlagiarismCheck.query.filter_by(doc_id=processed.id).order_by(PlagiarismCheck.check_date.desc()).first()
    if not check:
...
def export_report(report_id):
    ...
    if report.user_id != current_user.id:
        flash('Нет доступа')
        return redirect(url_for('student.dashboard'))
    # Заглушка: просто сообщаем, что экспорт "готов"
